...
def main():
    ...
    for a in alphas:
        ...
        _, q_adp, _, _, _ = compute_cefi_with_method(A, S, Sx, lambda A_, S_, s2_, q_, **kw: optimize_adaptive_armijo(A_, S_, s2_, q_, n_restarts=12, max_iter=100, **kw))
    ...
    for lam in lambdas:
        ...
        for w in wins:
            ...
            # Adaptive Armijo 12/100
            _, q_adp, _, _, _ = compute_cefi_with_method(
                A, S_eps, Sx_w,
                lambda A_, S_, s2_, q_, **kw: optimize_adaptive_armijo(A_, S_, s2_, q_, n_restarts=12, max_iter=100, **kw)
            )
