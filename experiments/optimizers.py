...
1. Fixed-step Riemannian Gradient Descent (audited production baseline).
2. Adaptive Riemannian Armijo line search with monotone increase and best-W tracking.
3. Riemannian Barzilai-Borwein (RBB) gradient method.
4. Wen & Yin (2013) feasible Cayley transform method.
"""
...
def optimize_fixed(A, S, s2, q, n_restarts=12, max_iter=100, lr=0.05,
    ...
    """
    Audited baseline: fixed-step Riemannian gradient descent without line search,
...
def optimize_adaptive_armijo(A, S, s2, q, n_restarts=12, max_iter=100, init_lr=0.05,
                             Sigma_x=None, seed_base=42, tol=1e-8, max_ls=30):
    """
    Adaptive Riemannian Armijo line search with monotone increase,
    best-W tracking across iterations, and gradient norm convergence check.
    """
    p = A.shape[0]
    inits = generate_initializations(p, q, n_restarts, Sigma_x=Sigma_x, seed_base=seed_base)
    ...
    return {
        ...
        "total_steps": total_steps,
        "elapsed_sec": elapsed,
        "method": f"adaptive_armijo_{n_restarts}_{max_iter}"
    }
