"""
Publication-Quality Figure Generator for Physical Review E
==========================================================
Generates Figures 1 to 4 in PDF and PNG formats:
- Fig 1: Scaling of attention to optimum vs operator Frobenius norm ||A||_F.
- Fig 2: Zero-cost diagnostic bracket on empirical regularization trajectory (103x gap).
...
# Style configuration for PRE (Physical Review E)
plt.rcParams.update({
    "font.size": 9,
    "font.family": "serif",
    "axes.labelsize": 9,
...
def plot_fig1():
    """Fig 1: Scaling of attention to optimum vs operator Frobenius norm."""
    df = pd.read_csv("experiments/operator_scaling.csv")
    
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(6.8, 2.7))
    
    # Left: Attention ratio to true optimum
    ax1.plot(df["frob_norm"], df["attn_adaptive"] * 100, 'o-', color='#1b7837', label='Adaptive Armijo (12/100)')
    ax1.plot(df["frob_norm"], df["attn_fixed"] * 100, 's--', color='#d73027', label='Fixed Step lr=0.05 (12/100)')
    ...
    ax1.set_ylabel(r'Attention to Optimum ($\hat{f} / f^*$, %)')
    ax1.set_ylim(-2, 108)
    ...
    plt.savefig("figures/fig1_operator_scaling.pdf", bbox_inches='tight')
    plt.savefig("figures/fig1_operator_scaling.png", dpi=300, bbox_inches='tight')
    plt.close()
    print("Generated figures/fig1_operator_scaling.pdf and .png")

def plot_fig2():
    """Fig 2: Zero-cost diagnostic bracket on empirical regularization trajectory."""
    ...
    ax1.set_ylabel(r'Causal Emergence Density $\Delta J$ (nats)')
    ax1.legend(loc='lower left', frameon=True, framealpha=0.9)
    ax1.set_title('(a) Diagnostic Bracket Sandwich')
    
    # Right: Underestimation ratio (Feasible Lower Bound / Reported)
    ...
    ax2.annotate(f'Peak {peak_row["bound_to_reported"]:.1f}x deficit\n'
                 ...
                 xytext=(peak_row["lambda_0"] * 2.5, peak_row["bound_to_reported"] * 0.55),
    ...
    plt.savefig("figures/fig2_diagnostic_bracket.pdf", bbox_inches='tight')
    plt.savefig("figures/fig2_diagnostic_bracket.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_fig3():
    ...
    ax1.plot(df_syn["alpha"], df_syn["q_adapt_12_100"], 'o-', color='#1b7837', label='Adaptive Armijo')
    ...
    ax1.set_ylabel(r'Selected Macro Dimension $q^*$')
    ax1.set_ylim(0.5, 3.5)
    ax1.legend(loc='upper right', frameon=True, framealpha=0.9)
    ax1.set_title(r'(a) Synthetic System ($p=30$)')
    
    # Right: Empirical system modal dimension shift across regularization
    ax2.plot(df_emp["lambda_0"], df_emp["modal_q_svd"], 'k--', label='SVD Bound Mode', linewidth=1.8)
    ax2.plot(df_emp["lambda_0"], df_emp["modal_q_adapt"], 'o-', color='#1b7837', label='Adaptive Armijo Mode')
    ax2.plot(df_emp["lambda_0"], df_emp["modal_q_fixed"], 's--', color='#d73027', label='Fixed 12/100 Mode')
    
    ax2.set_xscale('log')
    ax2.set_xlabel(r'Regularization Parameter $\lambda_0$')
    ax2.set_ylabel(r'Modal Macro Dimension $q^*$')
    ax2.set_ylim(-1, 31)
    ax2.legend(loc='center left', frameon=True, framealpha=0.9)
    ax2.set_title(r'(b) Empirical Trajectory ($p=30$)')
    
    plt.savefig("figures/fig3_dimension_corruption.pdf", bbox_inches='tight')
    plt.savefig("figures/fig3_dimension_corruption.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_fig4():
    ...
    ax1.plot(df_coup["g"], df_coup["ce_adaptive"], 'o-', color='#1b7837', label='Adaptive Armijo')
    ...
    ax2.set_title(r'(b) Attention to Optimum (%)')
    ...
    plt.savefig("figures/fig4_generalization_and_grid.pdf", bbox_inches='tight')
    plt.savefig("figures/fig4_generalization_and_grid.png", dpi=300, bbox_inches='tight')
    plt.close()
