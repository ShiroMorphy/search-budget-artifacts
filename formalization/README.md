...
| Theorem | Statement | Paper |
| :--- | :--- | :--- |
| `compression_quadratic_lower_bound` | If `W * Wᵀ = 1` and `c * (v ⬝ᵥ v) ≤ v ⬝ᵥ (S *ᵥ v)` for all `v`, then `c * (u ⬝ᵥ u) ≤ u ⬝ᵥ ((W * S * Wᵀ) *ᵥ u)` for all `u` | Rayleigh-Ritz step of Proposition 1 |

Taking `c` to be the least eigenvalue of the innovation covariance gives exactly
`λ_min (W Σ_ε Wᵀ) ≥ λ_min Σ_ε`. Stating it in the quadratic-form order rather
